...
class TransportApp(App):
    ...
    def __init__(self, routing_service: services.RoutingService, location_autocomplet_service: services.LocationAutocompletService, driver_class = None, css_path = None, watch_css = False):
    ...
    def compose(self) -> ComposeResult:
        yield Header()
        yield Footer()
        yield widgets.TransportWelcome(id="welcome")
    ...
    def watch_current_route(self, route: models.Route):
        ...
        try:
            self.query_one("#route-details").remove()
        except:
            pass

        self.mount(widgets.RouteWidget(route, id="route-details"))

    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        self.push_screen(screens.ConnectionDetailsScreen(self.current_route.connections[event.cursor_row]))
