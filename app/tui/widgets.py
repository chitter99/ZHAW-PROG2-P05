...
class RouteHeaderWidget(Widget):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Label(
            f"Coverage: {utils.parse_procent(self.route.best_coverage) if self.route.best_coverage else '100%'} SBB"
        )
        if self.route.service_end_countries:
            yield Label(
                f"Service ends in: {', '.join(self.route.service_end_countries)}"
            )
...
class ConnectionTableWidget(Widget):
    ...
    def on_mount(self) -> None:
        ...
        if self.route.only_direct_routes:
            table.add_columns(
                "Start", "Destination", "Departure", "Arrival", "Duration", "Transfers"
            )
            ...
            for connection in self.route.connections:
                table.add_row(
                    ...
                    connection.service_end_country,
                )
