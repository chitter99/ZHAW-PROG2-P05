...
from textual.widgets import Static, Label, Input, Button, Header, Footer, LoadingIndicator, Select
...
class CreateNewRouteScreen(BaseScreen):
    def compose(self) -> ComposeResult:
        ...
        with Container(id="route-form"):
            ...
            yield widgets.LocationAutocompletInput("Enter starting location", id="start")
            yield widgets.LocationAutocompletInput("Enter destination location", id="dest")
...
class LoadingRoutingScreen(Screen):
    ...
    def lookup_route(self) -> models.Route:
        return self.app.routing_service.route(models.RoutingParameters(
                start=self.start,
